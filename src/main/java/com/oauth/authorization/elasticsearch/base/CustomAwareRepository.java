...
public interface CustomAwareRepository<T, ID> extends Repository<T, ID> {
     ...
     */
    <S extends T> S save(String tenantId, S entity);
     ...
     */
    <S extends T> List<S> saveAll(String tenantId, List<S> entities);
     ...
     */
    void deleteAll(String tenantId, Iterable<? extends T> entities);
     ...
     * @return T 객체
     */
    T find(String tenantId, BoolQueryBuilder query);


    /**
     * findAll(String tenantId, BoolQueryBuilder query, List<QueryBuilder> filters, Sort sort, int pageSize)와 동일하지만
     * pageSize 를 기본 1_000으로 넣기 위한 래핑된 함수
     *
     * @param tenantId 회사 아이디를 넣은 경우 index_{namespace}_{companyIdLowerClase} 에서 도큐먼트를 찾는다.
     ...
     * @return T 객체 리스트
     */
    List<T> findAll(String tenantId, BoolQueryBuilder query, Sort sort);
     ...
     * @return T 객체 리스트
     */
    List<T> findAll(String tenantId, BoolQueryBuilder query, Sort sort, int pageSize);
     ...
     * @return SearchHits<T>
     */
    SearchHits<T> searchAfter(String tenantId, SearchAfterRequestDto searchAfterDto);
     ...
     * @return SearchHits<T>
     */
    SearchHits<T> searchAfterDetail(String tenantId, SearchAfterDetailRequestDto searchAfterDto);
     ...
     * @return bool
     */
    boolean bulkUpdate(String tenantId, List<UpdateQuery> updateQueryList);
}
